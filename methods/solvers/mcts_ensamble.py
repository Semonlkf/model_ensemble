...
class MCTSEnsambleSolverMixin:
    """
    Mixin class providing MCTS ensembling strategy.
    Follows the paper: AlphaZero-Like Tree-Search can Guide Large Language Model Decoding and Training
    """
    
    def solve_mctsem(self, x, idx, to_print=True):
        """
        Main MCTS ensembling method.
        """
        ...
        # Perform MCTS search
        solution_node = self.mctsem_mcts_alpha_search(
    ...
    def mctsem_mcts_alpha_search(self, root: MCTSNode, n_samples: int, num_simulation: int, c_base: float, c_init: float, max_depth: int, sample_action: bool, to_print: bool):
        """
        ...
        if not root.is_fully_expanded():
            self.mctsem_mcts_expand(node=current_node, n_samples=n_samples)
            ...
            for n in range(num_simulation):
                ...
                while node.is_fully_expanded() and not node.is_terminal():
                    if node.depth >= max_depth:
                        ...
                        break
                    node = self.mctsem_mcts_select_best_child(node, c_base, c_init)
                # Expansion Phase
                    if not node.is_terminal() and node.depth <= max_depth:
                        new_node = self.mctsem_mcts_expand(model_name=model_name, node=node, n_samples=n_samples)
                        # Backpropagation Phase
                        self.mctsem_mcts_backpropagate(new_node, new_node.MC_estimate)
            try:
    ...
    def mctsem_mcts_select_best_child(self, model_name: str, node: MCTSNode, c_base: float = 19652, c_init: float = 1.25):
        """
    ...
    def mctsem_mcts_expand(self, node: MCTSNode, n_samples: int):
        """
        ...
        if len(node.children) == 0:
            ...
            model_name = random.choice(llm_names)
            ...
            for action in actions:
                ...
                mc_estimate = self.get_values(model_name=self.args.backend_prm, x=x, ys=[ys])[0]
    ...
    def mctsem_mcts_simulate(self, model_name: str, state: str, current_depth: int, max_depth: int):
        """
    ...
    def mctsem_mcts_backpropagate(self, node: MCTSNode, value: float):
        """
    ...
    def mctsem_mcts_sample_action_based_on_visits(self, node: MCTSNode):
        """
        if model_name not in self.model_names:  
